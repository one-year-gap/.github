...
-[팀원 소개](#introduce-member)

---

<a id="overview"></a>
# 📌 프로젝트 소개 (Overview)
HOLLIVERSE는 통신 서비스 이용 데이터를 기반으로 고객 행동을 분석하고 운영자가 고객을 더 쉽게 이해할 수 있도록 돕는 분석 플랫폼입니다.

통신 서비스 환경에서는 고객 정보가 여러 시스템에 분산되어 있어 운영자가 고객 상태를 한 번에 파악하기 어렵습니다.

이 프로젝트는 다음 기능을 중심으로 설계되었습니다.
...
<a id="project-goal"></a>
# 프로젝트 Goal 
우리는 **일간 방문자 수 3만 명(Daily Visitors)을 목표**로 서비스를 기획하고 시스템을 설계하였습니다.

## 서비스 목표 트래픽 설정

본 프로젝트는 **LG U+ 모바일 고객 규모와 실제 통신사 웹 트래픽 데이터를 기반으로 목표 트래픽을 설정**하였습니다.

### 1️⃣ 통신사 고객 규모

LG유플러스의 전체 모바일 가입자는 약 **1,100만 명** 수준이며,  
이 중 **5G 가입자는 약 820만 명**으로 지속적인 성장세를 보이고 있습니다.  

즉, 통신사 서비스는 **수백만 명 단위의 대규모 사용자 기반**을 대상으로 하는 서비스이며,  
우리 서비스 역시 이와 같은 규모를 전제로 설계할 필요가 있습니다.

---

### 2️⃣ 실제 웹 트래픽 규모

트래픽 분석 플랫폼 데이터 기준으로, LG유플러스 공식 웹 서비스는 **월 약 1,100만 방문자**가 발생합니다.  
이를 일 단위로 환산하면 다음과 같습니다.

```text
월 방문자 수 ≈ 11,000,000명
일 평균 방문자 수 ≈ 11,000,000 / 30 ≈ 300,000명
```

즉, **일 평균 약 30만 명 수준의 트래픽**이 실제로 발생하고 있음을 알 수 있습니다.

---

### 3️⃣ 우리 서비스의 사용자 추정

LG U+ 온라인 채널 방문자는 다양한 목적을 가지고 유입됩니다.
...
본 프로젝트에서 제공하는 기능은  
**요금제 비교·탐색**, **현재 나의 요금제 현황 조회**, **상담/CS 연계**에 집중되어 있습니다.  

U+ 온라인몰 트래픽 분석을 바탕으로, 전체 방문자 중 약 **30%**가  
우리 서비스가 제공하는 영역(요금제 탐색/조회/상담)에 해당한다고 가정하였습니다.

```text
일 평균 전체 방문자 ≈ 300,000명
요금제 탐색·조회 관련 사용자 비중 ≈ 30%

→ 요금제 관련 사용자 수 ≈ 300,000 × 0.3 ≈ 90,000명
```

따라서 **요금제 관련 기능을 이용할 잠재 사용자 규모를 일 약 9만 명 수준**으로 추정하였고,  
이를 기반으로 **약 10만 명의 사용자가 존재**한다고 가정하여 서비스를 기획하였습니다.

---

### 4️⃣ 프로젝트 목표 사용자 (일간 방문자 수 3만 명)

위에서 추정한 요금제 관련 잠재 사용자(약 10만 명)를 기준으로,  
그 중 **하루에 실제로 서비스를 이용하는 활성 사용자 비율을 30% 수준**으로 설정하였습니다.

```text
추정 전체 잠재 사용자 ≈ 100,000명
하루 활성 사용자 비율(DAU 비율) ≈ 30%

→ 목표 일간 활성 사용자(DAU) ≈ 100,000 × 0.3 ≈ 30,000명
```

이에 따라 본 프로젝트는

- **일 방문자(DAU) 약 3만 명**
- **월 방문자(MAU) 약 90만 명 ≈ 100만 명**

의 트래픽을 **안정적으로 처리할 수 있는 시스템을 목표**로 삼았습니다.  

즉, **우리는 일간 방문자 수 3만 명을 기준 목표 트래픽으로 설정하고, 이 수준의 부하를 견딜 수 있는 아키텍처·인프라·데이터 설계를 수행**하였습니다.
----
<a id="tech-stack"></a>
## 🛠️ Tech Stack
...
* **Core:** Java 17 / Spring Boot 3.x 기반 도메인 중심 REST API 및 Bounded Context 모듈링
* **Security:** Spring Security + JWT + OAuth2 Client 연동 및 자체 토큰 관리
* **Data Access:** Spring Data JPA, **Querydsl**(동적 쿼리), **jOOQ**(통계/집계 리포트 최적화)
* **Batch & Messaging:** Spring Batch(일 단위 데이터 산출), Spring Kafka 기반 이벤트 스트리밍(AWS MSK 연동)
* **Strategy:** MapStruct/Lombok을 통한 생산성 향상, **TSID** 기반 샤딩 친화적 키 전략

---

### 🤖 Realtime AI / LLM Engine (Python)
<p>
...
* **Framework:** FastAPI + Uvicorn 기반 고성능 비동기 REST API
* **AI/LLM:** LangChain 기반 RAG 구성 및 상담 분석 파이프라인(OpenAI)
* **NLP:** spaCy 기반 한글 토큰화, rapidfuzz 등을 활용한 텍스트 매칭 최적화
* **Async Engine:** SQLAlchemy(asyncio) + aiokafka를 이용한 완전 비동기 데이터/이벤트 처리
...
* **RDBMS:** PostgreSQL(핵심 트랜잭션), pgvector(벡터 임베딩 검색/추천)
* **Management:** Flyway를 통한 DB 스키마 형상 관리, jOOQ Codegen을 이용한 타입 세이프 DSL 생성
...
* **Cloud:** AWS SDK v2, Secrets Manager(보안 설정), X-Ray(분산 트레이싱 및 병목 분석)
* **DevOps:** Docker(Multi-stage build), GitHub Actions CI/CD 파이프라인
* **Monitoring:** Spring Boot Actuator + Micrometer + Prometheus 메트릭 수집
* **Quality:** JUnit5, ArchUnit(아키텍처 검증), Jacoco(커버리지), Pytest(비동기 로직 검증)
...
<p align="center">
  <a href="https://www.notion.so" target="https://oxidized-cornflower-232.notion.site/2fd8a2b69065809fb671d47a8189b846?source=copy_link">
    <img src="https://img.shields.io/badge/Open_Notion_Space-000000?style=for-the-badge&logo=notion&logoColor=white" />
  </a>
  <a href="https://www.atlassian.com/software/jira" target="https://tkv00123.atlassian.net/jira/software/projects/HSC/boards/2/timeline">
    <img src="https://img.shields.io/badge/View_Jira_Board-0052CC?style=for-the-badge&logo=jira&logoColor=white" />
  </a>
  <a href="https://github.com" target="https://one-year-gap.github.io/docs/">
    <img src="https://img.shields.io/badge/Open_GitHub_Wiki-181717?style=for-the-badge&logo=github&logoColor=white" />
  </a>
</p>

* **Notion:** 문서화 및 프로젝트 지식 베이스 구축
* **Jira:** 애자일 스프린트 관리 및 이슈 트래킹
* **GitHub Wiki:** 기술 명세서 및 트러블슈팅 가이드 관리
...
### 1) 스플래시 페이지, 로그인 페이지, 약관 페이지, 회원가입 페이지
...
- 상단의 필터링을 통해 분류하여 조회가 가능합니다.
  1. 전체/추천 필터링: 전체 상품을 조회할 지, 본인의 캐릭터 유형에게 추천되는 상품제를 조회할 지 결정합니다.
  2. 상품제 카테고리 필터링: 5G/LTE, 태블릿/스마트워치, 부가서비스, IPTV, 인터넷
...
<img width="1346" height="874" alt="Image" src="https://github.com/user-attachments/assets/42efcc20-3e4e-4265-9b38-45ab1301f1da" />
...
- 상담 키워드 버블 차트
  ...
  2. 빈도 수가 높은 키워드일수록 버블의 크기가 커집니다.
